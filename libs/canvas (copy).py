...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if Qt.LeftButton & ev.buttons():
            if self.selectedVertex():
                radius = (((self.hShape[0].x() - self.hShape[-1].x())**2) + ((self.hShape[0].y() - self.hShape[-1].y())**2))**0.5
    ...
    def handleDrawing(self, pos):
        if self.current and self.current.reachMaxPoints() is False:
            ...
            self.current.addPoint(QPointF((maxX + minX) / 2 , (maxY + minY) / 2))
    ...
    def setLastLabel_2(self, text, line_color=None, fill_color=None, kind="Horizontal"):  # kind = vertical or horizontal
        assert text
        self.shapes[-1].label = text
