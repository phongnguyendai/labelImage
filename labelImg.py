...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        create = action('Create\nRectBox', self.createShape,
                        'n', 'new', u'Draw a new Box', enabled=False)

        # rotate = action('Rotate\nRectBox',self.rotateShape)

        delete = action('Delete\nRectBox', self.deleteSelectedShape,
                        'Delete', 'delete', u'Delete', enabled=False)
        copy = action('&Duplicate\nRectBox', self.copySelectedShape,
                      'Ctrl+c', 'copy', u'Create a duplicate of the selected Box',
                      enabled=False)
        ...
        # Shape.kind = flag_button_vertical(self.kind)
        Shape.kind = self.labelDialog.kind
        # print('Shape.kind', flag_button_vertical(self.kind))
    ...
    def btnstate(self, item= None):
        ...
        try:
            if difficult != shape.difficult:
                ...
                # print("item.checkState() == Qt.Checked: ", item.checkState() == Qt.Checked)
        except:
            pass
    ...
    def saveLabels(self, annotationFilePath):
        ...
        #shapes['kind'] = "xxx" # list shapes['kind']
    ...
    def newShape(self):
        ...
        # Add Chris
        self.diffcButton.setChecked(False)
        self.kind = self.labelDialog.getKind()
        # print(text, "\t\t\t*******gia tri cua text***********\t\t\t\t")
        # print("kind = ", self.kind)

        if text is not None:
            self.prevLabelText = text
            generate_color = generateColorByText(text)
            shape = self.canvas.setLastLabel_2(text, generate_color, generate_color,self.kind)
            self.addLabel(shape)
    ...
    def openPrevImg(self, _value=False):
        ...
        if self.filePath is None:
            return
        self.kind = self.labelDialog.kind
